...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt_check->execute([$nama_wilayah]);
    if ($stmt_check->fetch()) {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO wilayah (nama_wilayah) VALUES (?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nama_wilayah]);
            
            $_SESSION['success_message'] = "Wilayah baru berhasil ditambahkan.";
...
<head>
    ...
    <title>Tambah Wilayah - Billing ISP</title>
        ...
        <main class="p-4 md:p-8 flex-1 overflow-y-auto">
            <div class="max-w-lg mx-auto">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl md:text-3xl font-bold text-gray-800">Tambah Wilayah Baru</h2>
                ...
                <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg">
                    ...
                    <form action="add_wilayah.php" method="POST">
                        <div class="mb-5">
                            ...
                            <input type="text" id="nama_wilayah" name="nama_wilayah" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-3" placeholder="Contoh: Wilayah 1, Sektor A, dll." value="<?php echo htmlspecialchars($nama_wilayah); ?>" required>
                        </div>
