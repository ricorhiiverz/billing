...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO packages (name, price, mikrotik_profile) VALUES (?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$name, $price, $mikrotik_profile]);
            
            $_SESSION['success_message'] = "Paket baru berhasil ditambahkan.";
...
<head>
    ...
    <title>Tambah Paket - Billing ISP</title>
        ...
        <main class="p-4 md:p-8 flex-1 overflow-y-auto">
            <div class="max-w-lg mx-auto">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl md:text-3xl font-bold text-gray-800">Tambah Paket Baru</h2>
