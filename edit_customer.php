...
$customer_id_db = $_GET['id'];

// Ambil data customer yang akan diedit
$stmt = $pdo->prepare("SELECT * FROM customers WHERE id = ?");
$stmt->execute([$customer_id_db]);
$customer = $stmt->fetch();
...
$packages = $pdo->query("SELECT id, name, price, mikrotik_profile FROM packages ORDER BY name ASC")->fetchAll();
...
// Inisialisasi variabel dari data customer
...
$wilayah_id = $customer['wilayah_id'];
$errors = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        try {
            ...
            $sql_customer = "UPDATE customers SET 
                                name = ?, address = ?, phone_number = ?, package_id = ?, router_id = ?, 
                                access_method = ?, pppoe_user = ?, pppoe_password = ?, static_ip = ?, 
                                mac_address = ?, registration_date = ?, is_active = ?, wilayah_id = ? 
                             WHERE id = ?";
            $stmt_customer = $pdo->prepare($sql_customer);
            $stmt_customer->execute([
                $name, $address, $phone_number, $package_id, $router_id, 
                $access_method, $_POST['pppoe_user'], $_POST['pppoe_password'], $_POST['static_ip'], 
                $_POST['mac_address'], $registration_date, $is_active, $wilayah_id,
                $customer_id_db
            ]);
...
<head>
    ...
    <title>Edit Pelanggan - Billing ISP</title>
        ...
        <main class="p-4 md:p-8 flex-1 overflow-y-auto">
            <div class="max-w-4xl mx-auto">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl md:text-3xl font-bold text-gray-800">Edit Pelanggan</h2>
                ...
                <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg">
                    ...
                    <?php endif; ?>
                    <form action="edit_customer.php?id=<?php echo $customer_id_db; ?>" method="POST" id="customerForm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                             <div>
                                ...
                                <input type="text" id="customer_number" name="customer_number" class="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-3" value="<?php echo htmlspecialchars($customer_number); ?>" readonly>
                            </div>
                             <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <select id="package_id" name="package_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-3" required>
                                    <?php foreach($packages as $package): ?>
                                        <option value="<?php echo $package['id']; ?>" <?php if($package_id == $package['id']) echo 'selected'; ?>><?php echo htmlspecialchars($package['name']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                             <div>
                            ...
                            <div>
                                <label for="is_active" class="block mb-2 text-sm font-medium text-gray-600">Status Pelanggan</label>
                                <select id="is_active" name="is_active" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-3" required>
                                    <option value="1" <?php if($is_active == 1) echo 'selected'; ?>>Aktif</option>
                                    <option value="0" <?php if($is_active == 0) echo 'selected'; ?>>Non-Aktif</option>
                                </select>
                            </div>
                        </div>
